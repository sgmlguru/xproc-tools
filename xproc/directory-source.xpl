...
<p:declare-step
  ...
  <p:documentation xmlns="http://www.w3.org/1999/xhtml">
    <p>Use a directory as a source. We are wrapping an optionally recursive directory lister to load
      all documents in the listing. Failure to parse can be either ignored or cause an exception,
    ...
    <p:documentation xmlns="http://www.w3.org/1999/xhtml">
      <p>The <code>include-filter</code> option allows an option <em>regular expression</em> to be
        applied to either the file name or path name (depending on the value of
          <code>match-path</code> option). If the the match is successful, the file is retained
        unless excluded by the <code>exclude-filter</code> option.</p>
  ...
  <p:option name="exclude-filter" as="xs:string*" required="false">
    <p:documentation xmlns="http://www.w3.org/1999/xhtml">
      <p>The <code>exclude-filter</code> option allows an option <em>regular expression</em> to be
        applied to either the file name or path name (depending on the value of
          <code>match-path</code> option). If the the match is successful, the file is excluded from
        the results. The <code>exclude-filter</code> is applied after the
  ...
  <!--<p:option name="match-path" select="'false'">
    <p:documentation xmlns="http://www.w3.org/1999/xhtml">
      <p>The <code>match-path</code> option determines whether or not the
        ...
        whole path or just the file name. If set to <strong>true</strong> (case is insignificant)
  ...
  <p:option name="fail-on-error" select="'true'">
    <p:documentation xmlns="http://www.w3.org/1999/xhtml">
      <p>If this option is set to <code>true</code> (the default), then an exception will be raised
        when a loaded document cannot be parsed. If this option is set to false, a message will be
  ...
  <p:choose name="choose-load-docs">
    ...
    <p:when test="lower-case($fail-on-error) = 'true' and not($test = 0)">
      ...
      <p:for-each name="iterate-directory-results">
        ...
        <p:try name="try-load">
          ...
          <p:group>
            ...
            <p:load name="load-doc" parameters="map{'dtd-validate': true()}">
        ...
        <p:choose name="did-it-load">
          ...
          <p:when test="lower-case($fail-on-error) = 'true' and not($test = 0)">
            ...
            <p:load name="load-doc" parameters="map{'dtd-validate': false()}">
              <p:with-option name="href" select="/c:file/@uri">
